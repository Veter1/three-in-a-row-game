...
let permissionToPlay = true, scoreToWin = 1000, windowLoadSpeed = 1000;
onLoadApp();
...
gameWindow.addEventListener('click', async (event)=> {
    if (event.target.classList.contains('rock') && permissionToPlay === true)
        await onClickRockFunction(event)

    else if (event.target.classList.contains('btnRestart') && permissionToPlay === true){
    ...
    else if (event.target.classList.contains('btnBackToMenu') && permissionToPlay === true)
...
gameWindow.addEventListener('mouseout', (event)=>{
    if (event.target.classList.contains('btnRestart') && permissionToPlay === true && permissionToClick === true)
        event.target.style = event.target.style.cssText + "transform: scale(1)";
})
